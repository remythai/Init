...
export const getFieldId = (label: string): string => {
  return label
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
...
export const validateFieldDefinition = (field: Record<string, unknown>): boolean => {
  ...
  if (!field.type || !Object.values(FIELD_TYPES).includes(field.type as typeof FIELD_TYPES[keyof typeof FIELD_TYPES])) {
    throw new ValidationError(`Type de champ invalide. Types acceptés: ${Object.values(FIELD_TYPES).join(', ')}`);
  }

  if (['radio', 'select', 'multiselect'].includes(field.type as string)) {
    if (!Array.isArray(field.options) || field.options.length === 0) {
      throw new ValidationError(`Le champ de type ${field.type} doit avoir un tableau d'options non vide`);
    }

    (field.options as unknown[]).forEach((option, index) => {
...
export const validateCustomFields = (customFields: unknown): boolean => {
  if (!Array.isArray(customFields)) {
    throw new ValidationError('custom_fields doit être un tableau');
  }

  const labels = new Set<string>();
  customFields.forEach((field: Record<string, unknown>, index: number) => {
    try {
      validateFieldDefinition(field);

      const fieldId = getFieldId(field.label as string);
      if (labels.has(fieldId)) {
        throw new ValidationError(`Label dupliqué: ${field.label}`);
      }
      labels.add(fieldId);
    } catch (error) {
      throw new ValidationError(`Erreur dans le champ ${index}: ${(error as Error).message}`);
    }
  });
...
export const validateCustomData = (customFields: CustomFieldDefinition[], customData: Record<string, unknown>): boolean => {
  ...
  customFields.forEach(field => {
    ...
    switch (field.type) {
      ...
      case FIELD_TYPES.SELECT:
        if (!(field as CustomFieldDefinition & { options: string[] }).options.includes(value as string)) {
          errors[fieldId] = 'Valeur invalide';
        }
        break;

      case FIELD_TYPES.MULTISELECT: {
        const msField = field as CustomFieldDefinition & { options: string[] };
        if (!Array.isArray(value)) {
          errors[fieldId] = 'Doit être un tableau';
        } else {
          const invalidValues = value.filter((v: unknown) => !msField.options.includes(v as string));
