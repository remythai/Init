...
import type { RefreshTokenRow, UserType, DbClient } from '../types/index.js';
...
export const TokenModel = {
  async create(entityId: number, token: string, expiry: Date, userType: UserType = 'user', client: DbClient = pool): Promise<RefreshTokenRow> {
  ...
  async findValidTokenForUpdate(token: string, client: DbClient): Promise<RefreshTokenRow | undefined> {
    const tokenHash = hashToken(token);
    const result = await client.query(
      'SELECT * FROM refresh_tokens WHERE token = $1 AND expiry > NOW() FOR UPDATE',
      [tokenHash]
    );
    return result.rows[0];
  },

  async delete(token: string, client: DbClient = pool): Promise<void> {
    const tokenHash = hashToken(token);
    await client.query('DELETE FROM refresh_tokens WHERE token = $1', [tokenHash]);
  },
